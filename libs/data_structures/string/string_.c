...
void get_string_without_words_like_last_word(char *string, char *result) {
    ...
    if (_bag.size == 0) {
        ...
        for (int i = 0; i < _bag.size - 1; i++) {
            if (!are_two_words_equal(_bag.words[i], _bag.words[_bag.size - 1])) {
...
void test_get_word_preceding_first_common_word() {
    ...
    char s1_2[] = "";
    WordDescriptor w1;
    assert(get_word_preceding_first_common_word(s1_1, s1_2, &w1) == WORD_PRECEDING_EMPTY_STRING);

    char s2_1[] = "one three five";
    char s2_2[] = "zero two four";
    WordDescriptor w2;
    assert(get_word_preceding_first_common_word(s2_1, s2_2, &w2) == WORD_PRECEDING_NOT_FOUND_COMMON_WORD);

    char s3_1[] = "zero one five";
    char s3_2[] = "five three zero";
    WordDescriptor w3;
    assert(get_word_preceding_first_common_word(s3_1, s3_2, &w3) == WORD_PRECEDING_FIRST_WORDS_IS_COMMON);

    char s4_1[] = "zero one five";
    char s4_2[] = "one five";
    WordDescriptor w4;
    assert(get_word_preceding_first_common_word(s4_1, s4_2, &w4) == WORD_PRECEDING_WORD_FOUND);
